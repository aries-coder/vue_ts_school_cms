...
const useSystemStore = defineStore('student', {
  ...
  actions: {
    ...
    async requestCreatePageInfoAction(pageName: string, data: any) {
      ...
      const res = await requestCreatePageInfo(loginStore.accountKey, pageName, data)
      this.requestPageListInfoAction(pageName, {})
      window.$message.success(res)
    },
    ...
    async requestOtherListInfoAction() {
      const teacherRes = await requestPageListInfo(`/${loginStore.accountKey}/teacher/list`, {})
      this.teacherList = teacherRes

      const departmentRes = await requestPageListInfo(`/${loginStore.accountKey}/department/list`, {})
      this.departmentList = departmentRes

      const classRes = await requestPageListInfo(`/${loginStore.accountKey}/class/list`, {})
